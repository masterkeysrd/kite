...
func TestInput_ClickToSetCursor(t *testing.T) {
	...
	// Click on 'e' (offset 1)
	eng.ProcessRawEvent(&event.RawMouseEvent{X: 1, Y: 0, Button: event.ButtonLeft, Up: false})
	if off := inp.Buffer().ByteOffset(); off != 1 {
		t.Errorf("Click at (1,0) expected offset 1, got %d", off)
	}

	// Click on ' ' (offset 5)
	eng.ProcessRawEvent(&event.RawMouseEvent{X: 5, Y: 0, Button: event.ButtonLeft, Up: false})
	...
	eng.ProcessRawEvent(&event.RawMouseEvent{X: 15, Y: 0, Button: event.ButtonLeft, Up: false})
...
func TestTextArea_ClickToSetCursor(t *testing.T) {
	...
	eng.ProcessRawEvent(&event.RawMouseEvent{X: 0, Y: 1, Button: event.ButtonLeft, Up: false})
	if off := txa.Buffer().ByteOffset(); off != 6 {
		t.Errorf("Click at (0,1) expected offset 6, got %d", off)
	}

	// Click past the end of line 2
	eng.ProcessRawEvent(&event.RawMouseEvent{X: 10, Y: 1, Button: event.ButtonLeft, Up: false})
