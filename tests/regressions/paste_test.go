...
func TestPaste_CtrlV(t *testing.T) {
	...
	env.Engine.ProcessRawEvent(&event.RawBracketedPaste{Text: "pasted text"})
	env.Flush()
	...
	env.Engine.ProcessRawEvent(&event.RawBracketedPaste{Text: "alt+v text"})
	env.Flush()
	...
	env.Engine.ProcessRawEvent(&event.RawBracketedPaste{Text: "direct clipboard event"})
	env.Flush()
