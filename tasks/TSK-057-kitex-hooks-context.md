...
## Description
Implement the core generic Functional Component wrappers (`react.FC`, `react.FCWithChildren`) and the internal hook execution stack for the `kitex` package.

## Requirements
- **Directory Structure:** Create `extras/kitex/react/` (or place directly in `extras/kitex/`).
