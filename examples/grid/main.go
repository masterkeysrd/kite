...
var (
	gridContainerStyle         = style.S().Width(style.Percent(100)).Height(style.Percent(100)).Padding(style.Edges(1, 2)).Background(color.RGBA{15, 15, 15, 255})
	titleStyle                 = style.S().Foreground(color.RGBA{255, 255, 0, 255}).Margin(style.Edges(0, 0, 1, 0))
	...
	spanningGridStyle          = style.S().Display(style.DisplayGrid).GridTemplateColumns(style.Repeat(2, style.Fr(1))).GridTemplateRows(style.Repeat(2, style.Fr(1))).Height(style.Auto).Border(style.SingleBorder()).Gap(style.Gap(1))
	autoPlacementGridStyle     = style.S().Display(style.DisplayGrid).GridTemplateColumns(style.Repeat(3, style.Fr(1))).GridColumnGap(4).GridRowGap(1).Height(style.Auto).Border(style.SingleBorder()).Padding(style.Edges(1))
	mixedTracksGridStyle       = style.S().Display(style.DisplayGrid).GridTemplateColumns([]style.GridTrackSize{style.Cells(10), style.Auto, style.Fr(1)}).Height(style.Auto).Border(style.SingleBorder())
	...
	holyGrailGridStyle         = style.S().Display(style.DisplayGrid).GridTemplateColumns([]style.GridTrackSize{style.Cells(12), style.Fr(1), style.Cells(10)}).GridTemplateRows([]style.GridTrackSize{style.Cells(3), style.Fr(1), style.Cells(3)}).Width(style.Percent(100)).Height(style.Cells(15)).Border(style.SingleBorder())
)
