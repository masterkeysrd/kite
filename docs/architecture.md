...
- **Test Environment (`/testenv`):** A headless testing harness that wraps the existing `backend/mock`. Provides high-level APIs for structural DOM assertions (`GetNodeByID`), simulated input routing (`Type`, `Click`), layout verification, and golden/visual snapshot testing (producing HTML or ANSI dumps).
