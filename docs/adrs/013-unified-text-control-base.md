...
## Context
As we developed `<input>` (single-line) and `<textarea>` (multi-line) elements, we encountered massive code duplication. Both components manage an internal `text.Buffer`, an internal UA `uaDiv` block, and nearly identical implementations for:
...
1. **Shared State**: The base manages the `*text.Buffer`, the `doc` reference, the `uaDiv` block element, and scroll/focus state flags.
